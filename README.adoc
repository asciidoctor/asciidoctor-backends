...
[options="header", cols="1a,2m,2m"]
|===
|Backend    |`${presentation_lib}`    |`${template_directory}`
...
|asciidoctor-backends/slim/revealjs
...
* Follow the "General Usage Instructions", replacing `${presentation_lib}` and `${template_directory}` with the corresponding values from Table 1 above.
